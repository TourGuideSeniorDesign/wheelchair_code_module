...
int main(int argc, char *argv[]) {
  ...
  /* Initialize logger */
  Logger::init(nullptr, "https://autogiro-test-api.noah.dev/api/logs",
  ...
  auto electron_sub = node->create_subscription<std_msgs::msg::Int32>(
      "electron_selfdrive", 10, [node](std_msgs::msg::Int32::SharedPtr m) {
  ...
  while (rclcpp::ok()) {
    ...
    if (sel == 1) {
      ...
      if (!f_ok && cmd.leftSpeed > 0)
        cmd.leftSpeed = 0;
      if (!f_ok && cmd.rightSpeed > 0)
        cmd.rightSpeed = 0;
      if (!b_ok && cmd.leftSpeed < 0)
        cmd.leftSpeed = 0;
      if (!b_ok && cmd.rightSpeed < 0)
        cmd.rightSpeed = 0;
