...
int main(int argc, char *argv[])
{
    ...
    while (rclcpp::ok())
    {
        ...
        if (drive_mode.load() == 2) {
            ...
            bool right_ok = right_turn_clear.load();
            ...
            case Mode::PIVOT:
                if(pivot_dir == -1){//pivot left
                    ...
                    if(!right_ok) { mode = Mode::STOP; break; }
                    timer += DT;
                    cmd.rightSpeed  = (pivot_dir == +1) ? SPEED       : INNER_SPEED;
                    cmd.leftSpeed = (pivot_dir == +1) ? INNER_SPEED : SPEED;
                    if (timer >= PIVOT_DURATION) {
                        pivot_dir = -pivot_dir;          // swing back left
                        timer     = 0;
                        mode      = Mode::RETURN_PIVOT;
                    }
                }
            ...
            case Mode::RETURN_PIVOT:
                if(pivot_dir == 1){ //starting going left, now returning right
                    if (!left_ok) { mode = Mode::STOP; break; }
                    timer += DT;
                    cmd.leftSpeed  = (pivot_dir == +1) ? SPEED       : INNER_SPEED;
                    cmd.rightSpeed = (pivot_dir == +1) ? INNER_SPEED : SPEED;
                ...
                } else if (pivot_dir == -1){ //starting going right, now returning left
                    if (!right_ok) { mode = Mode::STOP; break; }
                    timer += DT;
                    cmd.rightSpeed  = (pivot_dir == +1) ? SPEED       : INNER_SPEED;
                    cmd.leftSpeed = (pivot_dir == +1) ? INNER_SPEED : SPEED;
